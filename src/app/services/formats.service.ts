...
export class FormatsService {
  ...
  carolinaBMLT = 'https://bmlt.sezf.org/main_server/client_interface/json/?switcher=GetFormats&show_all=1&format_ids=';

  constructor(private httpCors: HTTP) {}

  async getFormatNamesByID(uniqueIDs: Set<string>, language) {
    const formatNamesByID = {};
    const formatsApi = this.carolinaBMLT + Array.from(uniqueIDs).join(",");
