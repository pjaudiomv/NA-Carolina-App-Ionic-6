...
export class MeetingListProviderService {
  ...
  tomatoCarolinaBMLT = environment.tomatoCarolinaBMLT;

  constructor(public http: HttpClient) {
    console.log('Hello MeetingListProvider Provider');
  }

  getApiUrlMap: string = this.tomatoCarolinaBMLT + '&sort_keys=longitude,latitude,weekday_tinyint,start_time';
  getApiUrlDay: string = this.tomatoCarolinaBMLT + '&sort_keys=weekday_tinyint,start_time';
  ...
  getCircleMeetings(lat: string, long: string, radius: string) {
    const getApiUrlCircleMap: string = this.tomatoBMLT
      ...
      + '&long_val=' + long
      + '&lat_val=' + lat
      + '&sort_keys=longitude,latitude&callingApp=na-carolina-ionic';
  ...
  getMeetingsByAreaProvider(areaID: string) {
    const getMeetingsByAreaURL: string = this.tomatoBMLT
      ...
      + areaID
      + '&sort_keys=weekday_tinyint,start_time&callingApp=na-carolina-ionic';
    return this.http.get(getMeetingsByAreaURL);
  }

  getCounties() {
    const getCountiesURL: string = this.tomatoCarolinaBMLT
      + '&data_field_key=location_sub_province&sort_keys=location_sub_province';
    return this.http.get(getCountiesURL);
  }

  getMeetingsByCountyProvider(countyName: string) {
    const getCountyMeetingsURL: string = this.tomatoCarolinaBMLT
      + '&data_field_key=location_sub_province&sort_keys=location_sub_province';
  ...
  getSingleMeetingByID(id: string) {
    ...
    getCarolinaTomatoMeetings() {
      return this.http.get(this.tomatoCarolinaBMLT);
    }
}
